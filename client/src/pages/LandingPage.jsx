...
export default function LandingPage() {
  return (
    <div className="h-screen w-full bg-[#fcfdfd] text-slate-800 font-sans overflow-hidden flex flex-col relative">
      ...
      {/* Top Navigation */}
      <div className="w-full px-4 lg:px-8 pt-4 z-50 shrink-0">
        <nav className="w-full max-w-[1400px] mx-auto px-6 lg:px-8 py-3 lg:py-4 flex items-center justify-between bg-[rgba(255,255,255,0.05)] backdrop-blur-lg border border-[rgba(255,255,255,0.2)] shadow-[0_8px_32px_rgba(0,0,0,0.1)] rounded-2xl">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-brand-600 flex items-center justify-center text-white text-sm shadow-sm">
              🥦
            </div>
            <span className="text-xl font-bold text-slate-900 tracking-tight hidden sm:inline-block">Nutrition Finder</span>
          </div>
          
          <div className="hidden md:flex items-center gap-6 lg:gap-8 text-[13px] lg:text-sm font-medium text-slate-600">
          ...
          <Link to="/auth" className="flex items-center gap-2 border border-brand-200 bg-white/50 text-brand-600 px-4 lg:px-5 py-1.5 lg:py-2 rounded-full font-medium hover:bg-brand-50 transition-colors text-sm lg:text-base shadow-sm">
            <span className="text-base">👤</span> <span className="hidden sm:inline">Login / Sign In</span><span className="sm:hidden">Login</span>
          </Link>
        </nav>
      </div>
      ...
      <main className="w-full max-w-[1400px] mx-auto px-6 lg:px-12 flex-1 flex items-center relative z-10 min-h-0">
        ...
        <div className="hidden lg:flex w-1/2 h-full absolute right-0 top-0 items-center justify-end z-10 overflow-visible pointer-events-none">
          ...
          <div className="relative w-[70vh] max-w-[800px] aspect-square flex items-center justify-center translate-x-[15%]">
            ...
            <div className="absolute top-[10%] left-[5%] bg-white/95 backdrop-blur-sm rounded-xl lg:rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.06)] p-3 lg:p-4 border border-slate-50 w-48 lg:w-56 z-20 animate-float pointer-events-auto" style={{ animationDelay: '0s' }}>
            ...
            <div className="absolute top-[45%] -left-[10%] bg-white/95 backdrop-blur-sm rounded-xl lg:rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.06)] p-3 lg:p-4 border border-slate-50 w-48 lg:w-56 z-20 animate-float pointer-events-auto" style={{ animationDelay: '1.5s' }}>
              <div className="w-8 h-8 lg:w-10 lg:h-10 bg-green-50 rounded-lg flex items-center justify-center text-brand-600 mb-2 lg:mb-3">
                <svg className="w-4 h-4 lg:w-5 lg:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="absolute bottom-[10%] left-[5%] bg-white/95 backdrop-blur-sm rounded-xl lg:rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.06)] p-3 lg:p-4 border border-slate-50 w-40 lg:w-48 z-20 animate-float pointer-events-auto" style={{ animationDelay: '3s' }}>
              <div className="w-8 h-8 lg:w-10 lg:h-10 bg-green-50 rounded-lg flex items-center justify-center text-brand-600 mb-2 lg:mb-3">
                <svg className="w-4 h-4 lg:w-5 lg:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
